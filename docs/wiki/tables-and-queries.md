...
The implementation has one database adapter, a complete reservation slice, and a live scalar-field propagation experiment. This does not establish portability to another database or a materially different business domain. Relationships, indexes, authorization, transaction scope, retries, idempotency, and business policy remain explicit. Migration machinery can be shared; semantic migration intent cannot be inferred from a schema diff. ([Validation strategy](validation-strategy.md); [Project thesis](raw/project-thesis.md))
