...
`Commands.make({ name, group })` wraps a native Effect `RpcGroup` in an injectable service descriptor and `.layer` accepting a unary handler record or Effect. The group and its RPC definitions are retained, not regenerated from a second contract format. Authors use `Rpc.make(tag, { payload, success, error })` and explicit `Schema.toCodecJson` derivation for JSON wire schemas. Local service methods receive decoded payloads; captured layer dependencies serve as fallbacks, while the invocation context overrides them. `Application.make({ name, resources, commands })` joins descriptor and generated-resource groups and handler layers; use empty arrays for absent groups. `Application.prepare(application)` prepares the resource tables. ([Commands](../../src/commands.ts); [Book RPCs](../../examples/basic-crud/contracts.ts); [Application](../../src/application.ts); [command regressions](../../test/Commands.test.ts))
