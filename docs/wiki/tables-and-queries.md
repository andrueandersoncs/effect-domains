...
The minimal book application selects `Resource.crud` and has no authored contracts or handlers. The separate authored-SQL application shares its canonical `BookSchema` but intentionally declares different errors and a remove result containing the deleted row. Generated remove returns `void`; those semantic differences are not inferred. ([Basic resource](../../examples/basic-crud/resources.ts); [Canonical book](../../examples/basic-crud/domain.ts); [Authored contracts](../../examples/authored-sql/contracts.ts))
...
`Commands.make({ name, group })` wraps a native Effect `RpcGroup` in an injectable service descriptor and `.layer` accepting a unary handler record or Effect. The group and its RPC definitions are retained, not regenerated from a second contract format. `Commands.rpc(tag, { payload, success, error })` derives JSON codecs from the three explicit schemas and returns a native unary RPC, retaining decoded types and derived codec requirements. Native `Rpc.make` remains available for custom wire codecs and native options. Local service methods receive decoded payloads; captured layer dependencies are fallbacks, and invocation context overrides them. `Application.make({ name, resources, commands })` joins descriptor and generated-resource groups and handler layers; absent groups use empty arrays. `Application.prepare(application)` prepares the resource tables. ([Commands](../../src/commands.ts); [Authored RPCs](../../examples/authored-sql/contracts.ts); [Application](../../src/application.ts); [Codec regression](../../test/Commands.test.ts))

`ApplicationBun.run` is the sole Bun runner: it exposes generated remote operations and local `serve`, `schema`, and `inspect` commands. It builds the SQLite runtime, prepares resource tables through the migration store, builds services, runs initialization, then starts HTTP serving. Callers supply `Layer.empty` and `Effect.void` when those stages need no authored behavior. The database filename is an `Option`; `Option.none()` selects environment/default configuration. ([Bun runtime](../../src/application-bun.ts))
...
`SqliteMigrations.decodeHistory(raw)` decodes and validates fixture history as an Effect. Production startup can receive `{ migrations: decodedHistory, filename }`, or `{ manifest: absolutePath, filename }`, with an `Option` filename; `SqliteMigrations.load` reads ordered manifest artifacts. Fresh schemas are generated through `schema generate <name>` against an explicit manifest. ([Migrations](../../src/sqlite-migrations.ts); [Bun runtime](../../src/application-bun.ts))
