...
- `SqliteMigrations.history(raw)` decodes and validates fixture history; `SqliteMigrations.load(manifest)` reads the ordered artifact manifest. Every nonempty managed schema requires an initial migration, and the applied-artifact ledger is the sole migration state. `schema generate <name>` validates explicit `--rename`, `--backfill`, or `--transform` intent, writes a new artifact, then atomically replaces the manifest. A blocked or invalid plan leaves the registry unchanged. ([Migrations](../../src/sqlite-migrations.ts); [migration regressions](../../test/SqliteMigrations.test.ts))
...
The dated [simplification verification](validation-strategy.md#2026-09-08-simplification-verification) gives current exercised cases and test/type evidence. Older evidence remains historical rather than evidence of the current API.
