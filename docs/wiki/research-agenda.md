...
- `Commands.make({ name, contracts })` produces an injectable `Context.Service` descriptor, its generated RPC group, and `.layer(record | Effect)`. The layer captures fallback dependencies while invocation context takes precedence. `Application.make({ name, resources, commands })` merges resource and command handlers and detects duplicate unary operation names; empty arrays represent absent groups. `Application.prepare(application)` prepares resource tables. ([Commands](../../src/commands.ts); [Application](../../src/application.ts); [command regressions](../../test/Commands.test.ts))
