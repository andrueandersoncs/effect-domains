...
- `Resource.make({ name, schema, storage?, operations, create?, list? })` derives a storage table, canonical repository, selected RPC procedures, and handlers. `storage` is a reversible representation with the same decoded fields as the canonical schema, so storage codecs do not leak onto the wire. ([Resource](../../src/resource.ts); [resource regressions](../../test/ResourceCrud.test.ts))
...
- `Commands.make(name, contracts)` produces an injectable `Context.Service` descriptor, its generated RPC group, and `.layer(record | Effect)`. The layer captures fallback dependencies while invocation context takes precedence. `Application.make({ name, resources?, commands? })` merges resource and command handlers, detects duplicate unary operation names, and prepares resource tables. ([Commands](../../src/commands.ts); [Application](../../src/application.ts); [command regressions](../../test/Commands.test.ts))
- `ApplicationBun.run(application, { database: { manifest } | { migrations }, services?, initialize? })` is the single Bun entrypoint. It derives remote commands and local `serve`, `schema`, and `inspect` commands; server startup is database runtime, application preparation, services, optional initialization, then HTTP server launch. ([Bun runtime](../../src/application-bun.ts); [CLI](../../src/rpc-cli.ts))
- `SqliteMigrations.history(raw)` decodes and validates fixture history, while `SqliteMigrations.load(manifest)` reads the ordered artifact manifest. `schema generate <name>` requires a manifest, validates explicit `--rename`, `--backfill`, or `--transform` intent, writes a new artifact, then atomically replaces the manifest. A blocked or invalid plan leaves that registry unchanged. ([Migrations](../../src/sqlite-migrations.ts); [migration regressions](../../test/SqliteMigrations.test.ts))
...
Publication and runtime configuration remain outside canonical schemas. Intrinsic identity is domain metadata; generated persistence identity is an adapter concern. Command descriptors expose contracts without embedding RPC objects in application declarations. ([Domain identifier direction](raw/domain-identifier-and-basic-persistence-direction.md); [Commands](../../src/commands.ts))
...
Generated repositories handle routine persistence, while `Query.make` remains the explicit request/result escape hatch for authored behavior and codec-service dependencies. `PersistedRef.fromResource(resource, { key, ifMissing })` atomically finds or initializes one resource row, binds future commits to that key, and does not recreate a later-deleted row on refresh. It is local synchronization, not distributed cache coherence. ([Query](../../src/query.ts); [PersistedRef](../../src/persisted-ref.ts); [persisted-resource regression](../../test/PersistedResource.test.ts))

## Application Evidence

The reservation application uses canonical stock and reservation schemas with generated reads and explicit transactional reserve, confirm, release, accounting, and transition policy. The other five applications exercise generated todo CRUD, authored book queries, service-dependent storage codecs, resource-bound state, and a historical document migration. These are integration demonstrations, not a second materially different business-policy slice. ([Reservation resources](../../examples/reservations/resources.ts); [Reservation policy](../../examples/reservations/sqlite.ts); [Example guide](../../examples/README.md))

The dated [verification record](validation-strategy.md#2026-09-08-current-verification) gives the exercised cases and test/type evidence. Older evidence remains historical rather than evidence of the current API.
...
- how schema services and RPC middleware behave in an authenticated application;
...
The shared schema AST evaluator has exhaustive traversal and explicit cycle handling, but that does not imply every schema has a lossless table representation or all interpreters share target semantics. ([AST evaluator](../../src/schema-ast.ts); [AST regressions](../../test/SchemaAST.test.ts))
