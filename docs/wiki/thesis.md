...
Project implementations use Effect and narrow runtime interfaces. `Resource.define`, `Command.define`, `ReadModel.define`, and `Application.define` produce inspectable authoring syntax; they contain no table, repository, RPC group, handler, SQL executor, or adapter state. Explicit compiler boundaries derive those products. `Command.implement` is the authored Effect seam, and `Application.compile` produces the one authoritative `ApplicationIR` consumed by runtime, CLI, MCP, admin, and inspection adapters. ([Resource](../../packages/effect-domains/src/resource.ts); [Command](../../packages/effect-domains/src/command.ts); [ReadModel](../../packages/effect-domains/src/read-model.ts); [Application](../../packages/effect-domains/src/application.ts))
