...
Project implementations use Effect and narrow runtime interfaces. Native Effect RPCs define payload/success/error contracts; `Commands.rpc` derives JSON codecs, and `Commands.make({ name, group })` adds an injectable unary handler service. `Application.make({ name, resources?, commands? })` composes descriptors and resources. `ApplicationBun.run` returns the CLI/HTTP Effect; native `BunRuntime.runMain` executes fully provided application entrypoints. Transport selection remains separate from RPC declarations and canonical models. ([Commands](../../packages/effect-domains/src/commands.ts); [Book RPCs](../../apps/authored-sql/contracts.ts); [Application](../../packages/effect-domains/src/application.ts); [entrypoint](../../apps/basic-crud/main.ts))
