...
Project implementations use Effect and narrow runtime interfaces. Native `Rpc.make` and `RpcGroup.make` define operation contracts, and `Operation.make({ name, payload?, success, errors?, unavailable, policy?, transaction?, dependencies?, handler })` is the declarative seam over them: it applies JSON codecs, derives the public error union from domain `errors` plus `unavailable`, applies subject policy and transaction boundaries, records Resource/Table/view dependencies, and translates undeclared failures while the handler body stays an authored Effect. `Application.make({ name, parts })` composes Resources, `Operation.bundle` or native `{ group, handlers }` bundles, and nested Applications; there is no injectable command-service or context-capture wrapper. Transport selection remains separate from RPC declarations and canonical models. ([Operation](../../packages/effect-domains/src/operation.ts); [Application](../../packages/effect-domains/src/application.ts))
