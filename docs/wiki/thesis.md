...
Project implementations use Effect and narrow runtime interfaces. Native Effect RPCs define payload/success/error contracts. The authored-SQL slice uses `Rpc.make`, `RpcGroup.make`, and native handler layers directly; other applications still use `Commands.rpc` for JSON codecs and `Commands.make({ name, group })` for an injectable unary handler service. `Application.make({ name, resources?, commands? })` composes resources and native `{ group, handlers }` bundles as well as those descriptors. `ApplicationBun.run` returns the CLI/HTTP Effect; native `BunRuntime.runMain` executes fully provided entrypoints. Transport selection remains separate from RPC declarations and canonical models. ([Book RPCs](../../apps/authored-sql/contracts.ts); [native composition](../../apps/authored-sql/application.ts); [Commands](../../packages/effect-domains/src/commands.ts); [Application](../../packages/effect-domains/src/application.ts))
