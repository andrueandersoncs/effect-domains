...
Project implementations use Effect and narrow runtime interfaces. Native `Rpc.make` and `RpcGroup.make` define operation contracts; schemas needing a JSON representation use explicit `Schema.toCodecJson`. `RpcGroup.toLayer` installs authored handler Effects, with explicit error translation and transaction boundaries. `Application.make({ name, parts })` composes Resources, native `{ group, handlers }` bundles, and nested Applications; there is no injectable command-service, `Commands.make`, or RPC wrapper. Transport selection remains separate from RPC declarations and canonical models. ([Book RPCs](../../apps/authored-sql/contracts.ts); [reservation codecs](../../apps/reservations/contracts.ts); [Application](../../packages/effect-domains/src/application.ts))
