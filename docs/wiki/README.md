...
`Command.define` separates inspectable contract from `Command.implement` behavior. `ReadModel.define` and `ReadModel.fold` power joined schema/SQL metadata, dependencies, inspection, paging, and publication. `Application.define` uses explicit Part tags; `Application.compile` produces the sole `ApplicationIR` consumed by runtime, CLI, MCP, admin, and inspection. Orders/invoices now supplies domain-level evidence for `Part.application`: billing resources and commands are nested below an outer application that adds native identity. ([Command](../../packages/effect-domains/src/command.ts); [ReadModel](../../packages/effect-domains/src/read-model.ts); [Application](../../packages/effect-domains/src/application.ts); [billing composition](../../examples/orders-invoices/application.ts); [verification](validation-strategy.md#2026-09-14-nested-application-example-coverage))
