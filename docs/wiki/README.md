...
- [Validation Strategy](validation-strategy.md) — slice criteria, the current [compiler cutover record](validation-strategy.md#2026-09-14-explicit-compiler-architecture-cutover), native identity/browser/Cluster evidence, synchronization slices, prior validation, and dated evidence limits.
...
The later 2026-09-14 [declarative browser reactivity cutover](validation-strategy.md#2026-09-14-declarative-foldkit-reactivity-cutover) promotes reactive refetch from an isolated prototype into the real Foldkit runtime. `RpcBrowser.query` and `RpcBrowser.mutation` declare dependencies and invalidation keys over one runtime-provided native Reactivity service. Field Notes validates the authored authenticated/filter-dependent seam; Editorial Calendar validates mechanical `ResourceEditor` derivation from the exact Resource descriptor. Successful local mutations now refetch mounted lists without reducer reload commands. The boundary remains unary RPC with browser-local invalidation, not server push, cross-client notification, or offline synchronization.
...
The 2026-09-13 [client mechanics consolidation](validation-strategy.md#2026-09-13-client-mechanics-consolidation) moved the remaining repeated browser state machinery behind narrow framework helpers without deriving presentation or business policy. `BrowserModel` now owns shared notices, field errors, and form-failure translation; five more clients use `ResourcePager`; six authenticated reducers use `IdentitySession.embed`; and three materially different clients use `RpcBrowser.command`. All thirteen native bootstraps use the `ApplicationBun` runtime boundary and a single `StaticSpa.layerHttp` site declaration. Browser-entry and package-manifest generation were deliberately not added because their surviving declarations encode real application-specific resources, scripts, and dependencies. ([Implemented contract](research-agenda.md#implemented-contract))
...
`Command.define` separates inspectable contract from `Command.implement` behavior. `ReadModel.define` and `ReadModel.fold` power joined schema/SQL metadata, dependencies, inspection, paging, and publication. `Application.define` uses explicit Part tags; `Application.compile` produces the sole `ApplicationIR` consumed by runtime, CLI, MCP, admin, and inspection. ([Command](../../packages/effect-domains/src/command.ts); [ReadModel](../../packages/effect-domains/src/read-model.ts); [Application](../../packages/effect-domains/src/application.ts))
