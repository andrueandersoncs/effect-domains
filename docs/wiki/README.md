...
- [Validation Strategy](validation-strategy.md) — slice criteria, the current [deterministic lint remediation](validation-strategy.md#2026-09-22-deterministic-lint-backlog-remediation), native identity/browser/Cluster evidence, synchronization slices, prior validation, and dated evidence limits.
...
The current explicit compiler architecture cleanly separates intent from derived runtime values. `Resource.define`, `Command.define`, `ReadModel.define`, and `Application.define` produce inspectable syntax; `Resource.compile` and shared folds derive synchronous runtime products, while `Application.compile` returns a typed Effect yielding the authoritative `ApplicationIR`. Application modules execute compilation explicitly at their composition boundary, and every adapter consumes the resulting IR. Exact Resource identities survive composition, and the superseded Operation, SqliteView, and implicit application/resource constructors remain deleted. ([Implemented contract](research-agenda.md#implemented-contract); [architecture](tables-and-queries.md); [verification](validation-strategy.md#2026-09-14-explicit-compiler-architecture-cutover))
...
The 2026-09-22 semantic hardening made compiler and feature-flag definition failures explicit Effect channels, declared the package's source-only build contract, made strict compiler options local to every runtime workspace, and removed database causes, SQL details, physical columns, and constraint names from public failures. A dependency/ownership review confirmed that application parts and command dependencies are explicit descriptors while runtime requirements remain narrow Effect services or Layer environments. ([Verification record](validation-strategy.md#2026-09-22-semantic-contract-hardening))

The 2026-09-22 deterministic lint remediation completed the repository-wide Better TypeScript and TypeScript backlog after the native 0.4.25 executable fix. The 2026-09-23 clean cutover then removed every deterministic and semantic exclusion from the root configuration, replaced exempt facades and aliases with explicit owners, and reduced the exclusion-free package scan to zero semantic violation classifications. ([Verification record](validation-strategy.md#2026-09-23-exclusion-free-lint-remediation))

The framework package enforces a 500-line ceiling across `packages/effect-domains/src/**/*.ts` before deterministic lint. Its 77 source files separate resource, read-model, table, authorization, telemetry, migration, runtime, and transport responsibilities; package TypeScript and exclusion-free deterministic lint pass. ([Verification record](validation-strategy.md#2026-09-23-framework-source-line-limit))
