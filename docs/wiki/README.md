...
The [repair-workshop slice](../../examples/repair-workshop/README.md) supplies the first join/dependency evidence from that direction. `SqliteView` removes repeated storage codecs and SQL aliases/joins; `SqliteView.list` now derives its bounded typed filter/range payload, shared Page result, keyset cursor handler, and dependency declaration. The board uses that derived contract while authorization and transaction policy remain explicit. Broader joins and production identity remain unproven. ([Current contract](tables-and-queries.md#declared-joined-projections); [bounded regression](../../test/SqliteView.test.ts))
