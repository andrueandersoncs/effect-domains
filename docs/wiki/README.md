...
`serve` now derives MCP tools at `/mcp` alongside Effect RPC at `/rpc/v1`. Tool input/output schemas and execution come from the same operation contracts, with `{ input: payload }` and `{ result: success }` envelopes. Authentication remains per call through existing RPC middleware. ([MCP adapter](../../src/rpc-mcp.ts); [Client setup](../../examples/README.md#mcp-server); [MCP verification](validation-strategy.md#2026-09-09-mcp-runtime-derivation))
...
The earlier simplification removed the generic schema AST algebra, standalone table writes, database alias, framework Query wrapper, and custom command-contract representation. Authorization has its own closed policy AST and fold, not a general schema algebra. Type checking, lint, and all 36 tests across 13 files pass without exclusions. Current live checks cover MCP discovery/CRUD, protected note calls and denied-write preservation, and the unchanged RPC CLI. Earlier checks cover scoped pagination, physical note encoding, and historical todo ownership migration/restart. ([Current verification](validation-strategy.md#2026-09-09-mcp-runtime-derivation); [Authorized examples](validation-strategy.md#2026-09-09-authorized-examples))
