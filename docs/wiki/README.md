...
`Resource.make({ name, schema, authorization, operations, storage?, relations?, version?, transitions? })` retains declarative CRUD, optional patch and transition publication, explicit authorization, creation defaults, generation, typed subject bindings, and implicit null defaults. Every generated `list` returns bounded `{ items, nextCursor }`, defaults to limit 50, accepts declared equality filters and inclusive ranges, and follows the declared order with the identifier as tiebreak. Table and Authorization share scalar traversal while retaining distinct storage/canonical interpretations. Field products and closed operation descriptors feed storage, repositories, and native RPCs without a universal framework or parallel transport DTO. ([Resource](../../packages/effect-domains/src/resource.ts); [Transitions](../../packages/effect-domains/src/transitions.ts); [contract](tables-and-queries.md#generated-resource-operations))

`Application.make({ name, parts })` composes resources, `Operation.bundle` and native RPC bundles, and nested applications. `ApplicationBun.run` retains `services`, `initialize`, `background`, `routes`, and worker-only execution. Native execution layers compose through application-owned `services`; `SqliteBunRuntime.privateClient({ application, purpose })` opens identity or execution stores that provably differ from the application database. ([Application](../../packages/effect-domains/src/application.ts); [Bun runtime](../../packages/effect-domains/src/application-bun.ts); [private client](../../packages/effect-domains/src/sqlite-bun.ts); [native composition](tables-and-queries.md#native-durable-execution))
