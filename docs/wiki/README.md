...
Automatic [RPC telemetry](../reference/runtime.md#opentelemetry-tracing) installs native Effect OTLP tracing across Bun command lifetimes when an endpoint is configured. Reading List supplies the basic protobuf/Jaeger setup; Report Exports supplies HTTP/JSON metadata over a durable worker; Support Cases supplies explicit protobuf metadata across a nested application's server, Application UI, and CLI lifetimes. No endpoint means no collector traffic.
