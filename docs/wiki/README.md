...
Persistence now has separate implemented table and query definitions. `Table.make({ name, schema })` preserves its canonical source schema and derives a persisted `rowSchema`, adding an adapter-generated UUIDv7 `id` when no `identifier` annotation overrides it. `Query.make({ table, Request, Result, implementation })` defines one operation whose Effect requirements carry the runtime database dependency. `PersistedRef.make({ commit, load })` composes authored operations into a process-local write-through value with serialized fiber mutations, failure-safe publication, and explicit refresh. `SqliteBunRuntime` supplies table writes, UUIDv7 generation, and the database, while integration tests validate authored CRUD, generated identity, and query-backed persisted references. This is evidence for one persistence path, not proof of a general framework, cross-database portability, or multi-process synchronization. ([Tables and Queries](tables-and-queries.md); [Table implementation](../../src/table/algorithms.ts); [PersistedRef implementation](../../src/persisted-ref/types.ts); [Table and query API direction](raw/table-and-query-api-direction.md))
