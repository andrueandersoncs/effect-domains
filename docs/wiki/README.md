...
`Commands.make({ name, group })` takes a native Effect `RpcGroup`, retains it, and supplies an injectable service descriptor and `.layer` for unary handlers. Operations use `Rpc.make` with payload/success/error schemas and explicit JSON codec derivation; the former custom command-contract format is removed. `Application.make({ name, resources, commands })` combines descriptor and resource handlers; empty arrays represent absent groups. `Application.prepare(application)` prepares resource tables. One `ApplicationBun.run` entrypoint exposes generated remote commands plus `serve`, `schema`, and `inspect`; startup prepares the database, builds services, initializes, then launches the server. Runtime options use an `Option` filename, `Layer.empty` for no authored services, and `Effect.void` for no initialization. ([Commands](../../src/commands.ts); [Book RPCs](../../examples/basic-crud/contracts.ts); [Application](../../src/application.ts); [Bun runtime](../../src/application-bun.ts))
...
The simplification removes the generic AST algebra, opaque descriptor schema classes, standalone table writes, the database alias, and the framework Query wrapper. Authored SQL uses Effect primitives; native CLI flags remain. The native-RPC cutover also removes the custom command-contract representation. Type checking, lint, and all 24 tests across nine files pass without exclusions. Fresh live checks cover authored book CRUD, counter cache/refresh, and reservation transitions through the generated CLIs; earlier generated-resource checks remain separately dated evidence. ([Native RPC verification](validation-strategy.md#2026-09-08-native-rpc-contract-verification); [Lint remediation verification](validation-strategy.md#2026-09-08-lint-remediation-verification); [framework contract](research-agenda.md))
