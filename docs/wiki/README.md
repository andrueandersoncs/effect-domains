...
The [2026-09-09 lint remediation record](validation-strategy.md#2026-09-09-repository-wide-lint-remediation) records passing repository-wide lint, workspace and integration type checks, all 45 tests, and the admin build. Real CLI and browser checks cover CRUD, authentication, boolean filters, and cursor navigation. No lint rules were disabled. The earlier [simplification record](validation-strategy.md#2026-09-09-abstraction-simplification) retains its historical source counts and verification results.
...
`Commands.make({ name, group })` retains a native Effect `RpcGroup` and supplies an injectable descriptor. Its `.layer(handlers, catchTags)` maps selected tagged invocation errors, not acquisition failures, defects, or interruption; the mapper argument is optional. `Application.make` accepts omitted resource/command groups. `ApplicationBun.run` returns an Effect, which application entrypoints pass to native `BunRuntime.runMain`. Database manifests accept `string | URL`; filename, services, and initialization are optional. ([Commands](../../packages/effect-domains/src/commands.ts); [Application](../../packages/effect-domains/src/application.ts); [Runtime](../../packages/effect-domains/src/application-bun.ts))
