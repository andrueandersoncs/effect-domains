...
The following two changes exercise the application-level validation. Both commands exit nonzero; the encoded declared errors carry the shown tags and fields.

```bash
# The address and payload recipient disagree.
bun run appointment-reminders AppointmentRecipient.ScheduleReminderDiscard --input-json "{\"entityId\":\"not-alice\",\"payload\":{\"recipient\":\"alice\",\"reminderId\":\"$REMINDER_ID\",\"appointmentId\":\"$APPOINTMENT_ID\",\"appointmentAt\":\"$APPOINTMENT_AT\",\"reminderAt\":\"$REMINDER_AT\",\"location\":\"Studio A\",\"purpose\":\"Equipment handover\"}}"

# The reminder is not before the appointment.
bun run appointment-reminders AppointmentRecipient.ScheduleReminderDiscard --input-json "{\"entityId\":\"alice\",\"payload\":{\"recipient\":\"alice\",\"reminderId\":\"$REMINDER_ID\",\"appointmentId\":\"$APPOINTMENT_ID\",\"appointmentAt\":\"$REMINDER_AT\",\"reminderAt\":\"$APPOINTMENT_AT\",\"location\":\"Studio A\",\"purpose\":\"Equipment handover\"}}"
```

The first is `AppointmentRecipientMismatch` with `entityId` and `recipient`. The second is `ReminderMustPrecedeAppointment` with `appointmentId`, `appointmentAt`, and `reminderAt`. A database persistence problem is represented as `AppointmentReminderDeliveryFailed` with `recipient` and `reminderId`. Empty location/purpose, non-UUIDv7 IDs, and invalid UTC timestamps are rejected by the RPC schema before delivery.

## Access, web, and MCP

Both scheduling and the inbox resource use the subject-only admin policy. `admin-demo` may use the walkthrough; `alice-demo` is an editor but is denied, and the other public demo sessions are denied too. The recipient is data, not the authenticated subject, so the admin can schedule and query Alice's inbox.
...
Application data defaults to `appointment-reminders.sqlite`; the native execution store defaults to `appointment-reminders.execution.sqlite`; the projection defaults to `appointment-reminders.notifications.json`. The application and execution stores have separate transaction boundaries. Back up both databases for recovery, but do not infer a cross-database transaction, automatic outbox, or exactly-once delivery guarantee for arbitrary external systems. Inbox insertion is deduplicated in the application database; the scope of that behavior is the durable in-app row described here.
