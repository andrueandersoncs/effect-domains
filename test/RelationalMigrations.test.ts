...
const relationalLifecycle = Effect.fn("SqliteMigrations.relationalLifecycle")(function* () {
  ...
  yield* sqliteMigrationStore(sql, [initial]).prepare(source.tables)
  ...
  const invalidMigration = yield* pipe(sqliteMigrationStore(sql, [initial, addConstraints]).prepare(constrained.tables), Effect.result)
  ...
  const constrainedStore = sqliteMigrationStore(sql, [initial, addConstraints])
  ...
  const indexedStore = sqliteMigrationStore(sql, [initial, addConstraints, addIndex])
  ...
  const droppedStore = sqliteMigrationStore(sql, [initial, addConstraints, addIndex, dropIndex])
...
const referencedParentRebuild = Effect.fn("SqliteMigrations.referencedParentRebuild")(function* () {
  ...
  const store = sqliteMigrationStore(sql, [initial, rebuild])
